...
- **Payment & Subscriptions**: Stripe Checkout and signed webhooks.
...
- `STRIPE_SECRET_KEY` (`stripe_dev_*` only for the explicit local mock, `sk_test_*` for Stripe test mode, and `sk_live_*` in production).
- `STRIPE_WEBHOOK_SECRET` (the `whsec_*` endpoint signing secret).
- `STRIPE_MONTHLY_PRICE_ID` and `STRIPE_ANNUAL_PRICE_ID` (required for real Stripe checkout).
...
1. Open the Stripe Dashboard in the intended environment.
2. Create two recurring BRL prices matching the product spec:
   - **Monthly**: R$ 29,90
   - **Annual**: R$ 287,00
3. Add a Webhook Endpoint pointing to your production site:
   ...
   ```
4. Subscribe it to `checkout.session.completed`, `invoice.paid`,
   `invoice.payment_failed`, and `customer.subscription.deleted`.
5. Copy the `price_*` IDs and the endpoint `whsec_*` signing secret.

The current subscription flow accepts card payments. Stripe PIX does not support
recurring payments; adding PIX to the annual plan requires a separate one-time
payment flow that grants 12 months of access.
...
4. Complete a Stripe test-mode checkout and confirm the signed webhook activates access.
